...
// Do not edit the code below.
let family = ['Tyler', 'Jordyn', 'Ryan', 'Chelsey', 'Ireland'];
// Do not edit the code above.
...
let looper = (family) => { for(var i = 0; i < family.length; i++) {
  alert(family[i]);
  }
}
...
evenFinder = (nums) => nums.filter((item) => item % 2 === 0);
...
divider = (numbersArray) => [numbersArray.filter((item) => item % 2 === 0), numbersArray.filter((item) => item % 2 !== 0)];
...
var addItem = (myGroceryList, item) => {
  if(myGroceryList.indexOf(item) === -1) {myGroceryList.push(item)};
  return myGroceryList;
}
...
var addTen = (numbers) => numbers.map((item) => parseInt(item) + 10);
...
var both = (arr1,arr2) => arr1.filter((item) => arr2.indexOf(item) !== -1);
...
users.push(user1, user2 = { name: 'Sandshrew Smele', email: "asemle", password: "smelele", username: "asemle" }, user3 = { name: 'Sandshrew Smele', email: "asemle", password: "smelele", username: "asemle" });
